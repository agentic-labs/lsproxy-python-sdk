# Lsproxy Python API library

[![PyPI version](https://img.shields.io/pypi/v/lsproxy.svg)](https://pypi.org/project/lsproxy/)

The Lsproxy Python library provides convenient access to the Lsproxy REST API from any Python 3.7+
...
Simply import `AsyncLsproxy` instead of `Lsproxy` and use `await` with each API call:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `lsproxy.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `lsproxy.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `lsproxy.APIError`.
...
except lsproxy.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except lsproxy.APIStatusError as e:
...
You can enable logging by setting the environment variable `LSPROXY_LOG` to `debug`.

```shell
$ export LSPROXY_LOG=debug
```
...
```python
from lsproxy import Lsproxy, DefaultHttpxClient

client = Lsproxy(
    # Or use the `LSPROXY_BASE_URL` env var
