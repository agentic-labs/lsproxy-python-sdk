...
def __(hops, js_dropdown, lang_select, mo, py_dropdown, rs_dropdown):
    ...
    submit_button = mo.ui.run_button(
        ...
        disabled=not lang_dropdown.value
    )
...
def __(
    ...
    return api_client, builder, mermaid_string
...
def __(create_lang_dropdown, get_files):
    def create_dropdowns():
        ...
        file_with_symbol_count = [
            (file, len(symbols)) for file, symbols in file_dict.items() if len(symbols) > 0
        ]
        file_with_symbol_count = sorted(
            file_with_symbol_count, key=lambda item: -item[1]
        )
        py_dropdown = create_lang_dropdown(file_with_symbol_count, ["py"], "Python files")
        js_dropdown = create_lang_dropdown(file_with_symbol_count, ["ts", "tsx", "js", "jsx"], "Typescript/Javascript files")
...
def __(mo):
    def create_lang_dropdown(file_symbol_dict, endings, label):
        ...
        return mo.ui.dropdown(
            options=file_options, label=label
        )
    ...
    class GraphBuilder:
        ...
        def process_symbols_in_file(self, file):
            for symbol in mo.status.progress_bar(
                ...
                references = (
                    SymbolApi(self.api_client).find_references(get_references_request).references
                )
